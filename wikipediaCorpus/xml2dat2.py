...
XmlData = "/home/xfbai/corpus/monolingual/wikicomp-2014_enzh.xml"
# number of doc pair need to extract for training
pair_num_trn = 50000000
...
print "length of docs_ids is",str(len(doc_ids))
en = [en[i] for i in doc_ids if(i<len(en))]
zh = [zh[i] for i in doc_ids if(i<len(zh))]
'''
#######################################################################################
...
for i in range(len(zh)):
    with open('test_in.txt', 'w') as f:
        f.write(zh[i])
    os.system('cat test_in.txt | opencc -o test_out.txt -c t2s.json')
...
os.system('/home/xfbai/tools/stanford-segmenter-2017-06-09/segment.sh ctb zhs.txt UTF-8 0 > zhs_seg.txt')
